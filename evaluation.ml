  ...
  |For(paraList,argList,bk) ->
    ...
    begin
      ...
      |s::paraList1,(Cons(v1,v2))::vlist1 -> (* print_value (Cons(v1,v2)); *)
        (* 2要素目 *)
        let (v2,env2,tenv2) = expr_secondFor_eval paraList1 vlist1 bk (updateEnv env1 s (Any:Program.t) (Some v1)) tenv1 in
        (* 2周目以降 *)
  ...
  |For_dict(paraList,dict,bk) ->
    ...
    begin
      ...
      |Cons(v2,v3) ->
        ...
        let env5 = removeEnv env4 paraList in
        (Null,env5,tenv3)                   
      |_ -> raise Error
    end
...
and expr_secondFor_eval (paraList:string list) (vlist:Program.v list) (bk:Program.bk) (env:Program.env) (tenv:Program.tenv) :Program.evalResult =
  ...
  |s::paraList1,(Cons(v1,v2))::vlist1 -> expr_secondFor_eval paraList1 vlist1 bk (updateEnv env s (Any:Program.t) (Some v1)) tenv
                                       
  |s::paraList1,(Nil)::vlist1 -> expr_secondFor_eval paraList1 vlist1 bk (updateEnv env s (Any:Program.t) None) tenv
...
and expr_tval (e:Program.e) (env:Program.env list) (tenv:Program.tenv) (tequals:Program.tequals) (n:int) :Program.tvalResult =
  ...
  |Sub(e,p) ->
    ...
    let (tequals2,n2) = pat_tval p env tenv ((t (n+1),List (t (n1+1)))::tequals1) (n1+1) in
  ...
  |Match(e,patlist) ->
    ...
    begin
      ...
      |(p,bk)::[] ->
        ...
        let (tequals2,n2) = pat_tval p env1 tenv ((t (n+1),t (n1+1))::tequals1) (n1+1) in
  ...
  |For(paraList,argList,bk) ->
    begin
      ...
      |s1::paraList1,e1::argList1 ->
        ...
        block_tval bk env2 tenv ((t (n2+1),Unit)::tequals2) (n2+1)
      |_ -> raise Error
    end
          ...
          |Tuple elist ->
            begin
              ...
              |t1::tlist1 ->
                ...
                block_tval bk env1 tenv ((t (n1+1),Unit)::tequals1) (n1+1)
              |_ -> raise Error
            end
          |_ -> raise Error
        end
      ...
      |e::elist1 ->
        begin
          ...
          |Declrt1(t1,s,e1) ->
            ...
            block_tval (Block(elist1)) env1 tenv (((t n),t (n1+1))::tequals1) (n1+1)
          |Declrt2(t1,s) ->
            let lenv = (s,t1,None)::(find_remove (List.hd env) s []) in
            let env1 = (lenv::env) in
            block_tval (Block(elist1)) env1 tenv (((t n),t (n+1))::tequals) (n+1)
          |_ ->    
            let (tequals1,n1) = expr_tval e env tenv ((t (n+1),Unit)::tequals) (n+1) in
            block_tval (Block(elist1)) env tenv (((t n),t (n1+1))::tequals1) (n1+1)
        end
    end
...
and secondMatch_tval (patlist:(Program.p * Program.bk) list) (env:Program.env list) (tenv:Program.tenv) (tequals:Program.tequals) (n:int) (np:int) (ne:int) :Program.tvalResult =
  ...
  |(p,bk)::[] ->
    ...
    let env1 = (lenv::env) in
    let (tequals1,n1) = pat_tval p env1 tenv ((t np,t (n+1))::tequals) (n+1) in
